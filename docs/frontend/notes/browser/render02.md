...
```
表示资源在`2019年11月22号8点41`分过期，过期了就得向服务端发请求。
...
* 否则返回304，告诉浏览器直接用缓存。
#### 两者对比
1. 在`精准度上`，`ETag`优于`Last-Modified`。优于 ETag 是按照内容给资源上标识，因此能准确感知资源的变化。而 Last-Modified 就不一样了，它在一些特殊的情况并不能准确感知资源变化，主要有两种情况:
